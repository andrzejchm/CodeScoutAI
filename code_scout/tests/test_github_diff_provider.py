...
@pytest.fixture
def mock_github_service() -> Generator[MagicMock, Any, Any]:
...
@pytest.fixture
def mock_github_pull_request() -> MagicMock:
...
@pytest.fixture
def mock_github_file_modified() -> MagicMock:
...
@pytest.fixture
def mock_github_file_added() -> MagicMock:
    mock_file = MagicMock(spec=File)
...
@pytest.fixture
def mock_github_file_deleted() -> MagicMock:
    mock_file = MagicMock(spec=File)
...
@pytest.fixture
def mock_github_file_renamed() -> MagicMock:
    mock_file = MagicMock(spec=File)
...
class TestGitHubDiffProvider:
    ...
    def test_init_validation(self) -> None:
        with pytest.raises(ValueError, match="Repository owner cannot be empty."):
            _ = GitHubDiffProvider(
                repo_owner="", repo_name=self.REPO, pr_number=self.PR_NUMBER, github_token=self.TOKEN
            )
        with pytest.raises(ValueError, match="Pull request number must be a positive integer."):
            _ = GitHubDiffProvider(repo_owner=self.OWNER, repo_name=self.REPO, pr_number=0, github_token=self.TOKEN)
        with pytest.raises(ValueError, match="GitHub token cannot be empty."):
            _ = GitHubDiffProvider(
                repo_owner=self.OWNER, repo_name=self.REPO, pr_number=self.PR_NUMBER, github_token=""
            )

    def test_get_diff_modified_file(
        self,
        mock_github_service: MagicMock,
        mock_github_pull_request: MagicMock,
        mock_github_file_modified: MagicMock,
    ) -> None:
    ...
    def test_get_diff_added_file(
        self,
        mock_github_service: MagicMock,
        mock_github_pull_request: MagicMock,
        mock_github_file_added: MagicMock,
    ) -> None:
        mock_github_service.get_pull_request.return_value = mock_github_pull_request
    ...
    def test_get_diff_deleted_file(
        self,
        mock_github_service: MagicMock,
        mock_github_pull_request: MagicMock,
        mock_github_file_deleted: MagicMock,
    ) -> None:
        mock_github_service.get_pull_request.return_value = mock_github_pull_request
    ...
    def test_get_diff_renamed_file(
        self,
        mock_github_service: MagicMock,
        mock_github_pull_request: MagicMock,
        mock_github_file_renamed: MagicMock,
    ) -> None:
        mock_github_service.get_pull_request.return_value = mock_github_pull_request
    ...
    def test_get_diff_skips_unparseable_file(
        self,
        mock_parse_github_file: MagicMock,
        mock_github_service: MagicMock,
        mock_github_pull_request: MagicMock,
        mock_github_file_modified: MagicMock,
    ) -> None:
